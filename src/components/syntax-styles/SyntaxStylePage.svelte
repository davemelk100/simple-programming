...
{#if style}
  <div class="space-y-6">
    ...
    <div class="min-h-[300px] rounded-2xl bg-white shadow-sm border border-slate-100 p-4 md:p-6 lg:p-8 card-with-tabs">
      <div class="title-row">
        <h2 class="text-2xl font-bold text-slate-800" style="font-family: 'Permanent Marker', sans-serif;">{style.title}</h2>
        <SectionTabs
          {activeSection}
          {completedSections}
      ...
      {#if components[styleSlug]}
        ...
        {:else if activeSection === 'ai'}
          <AiSection topicTitle={style.title} topicSlug={styleSlug} oncomplete={() => handleSectionComplete('ai')} />
        {/if}
      {:else}
