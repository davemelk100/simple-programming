...
<nav class="flex h-full w-56 shrink-0 flex-col gap-0.5 overflow-y-auto border-r border-slate-200 bg-white px-3 py-4">
  ...
  {#each topics as topic}
    ...
    <a
      ...
      class="flex items-center gap-2 rounded-lg px-2 py-1.5 text-sm font-medium no-underline transition-colors {active ? `${colors?.active} font-semibold` : `text-slate-600 ${colors?.hover ?? 'hover:bg-slate-50'}`}"
