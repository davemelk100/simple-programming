...
<div class="space-y-8">
  <div class="text-center">
    ...
    <p class="mx-auto max-w-lg text-slate-600">
      Programs need to <strong>choose</strong> what to do and sometimes <strong>repeat</strong> actions. Let's see how!
    </p>
  </div>

  <!-- Decision tree -->
  <div class="transition-all duration-700 {showDecision ? 'opacity-100' : 'opacity-0'}">
    <h3 class="mb-4 text-center text-lg font-bold text-orange-700">If / Else: Making Choices</h3>
  ...
  <!-- Loop visual -->
  <div class="transition-all duration-700 {showLoop ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}">
    <h3 class="mb-4 text-center text-lg font-bold text-orange-700">Loops: Repeating Actions</h3>
    <p class="mb-3 text-center text-sm text-slate-600">"Say hello 5 times"</p>
      ...
      {#each [1, 2, 3, 4, 5] as step}
        <div class="flex flex-col items-center gap-1 transition-all duration-400
          ...
          <div class="flex h-12 w-12 items-center justify-center rounded-full border-2 font-bold transition-all duration-300
            {step <= loopStep ? 'border-orange-400 bg-orange-100 text-orange-700' : 'border-slate-200 bg-slate-50 text-slate-400'}">
            {step}
          </div>
  ...
  <div class="rounded-xl bg-orange-50 p-5 text-center">
    <p class="text-sm text-orange-800">
      <strong>If/else</strong> lets your program pick a path.
      <strong>Loops</strong> let your program repeat steps without writing the same code over and over!
    </p>
  </div>
