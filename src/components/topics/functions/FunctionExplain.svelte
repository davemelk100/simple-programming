    ...
    {#each examples as ex, i}
      <div
        class="flex w-full max-w-lg items-center gap-3 transition-all duration-700
          {i <= activeExample ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}"
      >
        <!-- Input -->
      ...
      {#if i <= activeExample}
        <p class="max-w-md text-center text-sm text-slate-500 -mt-4">{ex.desc}</p>
      {/if}
    {/each}
  </div>

  <div class="rounded-xl bg-purple-50 p-5 text-center">
