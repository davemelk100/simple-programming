<script lang="ts">
  ...
  const slides: Slide[] = [
    {
      ...
      metaphor: "🏗️",
      description:
        "Before anything else, the building must exist. Variables, functions, data structures, control flow, and object-oriented thinking are the concrete and steel of every application.",
      insight: "Foundation",
    ...
    },
    {
      ...
      metaphor: "⚙️",
      description:
        "Before the doors open, the bank sets its rules. Syntax styles, naming conventions, DRY, KISS, and planning are the policies that govern how every line of code behaves.",
      insight: "Policy",
    ...
    },
    {
      ...
      metaphor: "🗄️",
      description:
        "The vault is stocked and the ledger is open. Real languages — TypeScript, Python, Go, C# — plus version control, the command line, and local servers. This is where you store and manage real code.",
      insight: "Execution",
    ...
    },
    {
      ...
      metaphor: "🧠",
      description:
        "Now you hire the employees. Choose a stack, scaffold a project, connect a database, add auth, build an API, and deploy. These are the brains that make your application actually work.",
      insight: "Creation",
      ...
      icon: "🧠",
      walkthrough:
        "Step 4: The employees are trained. The bank is open for business.",
    ...
    },
    {
      ...
      metaphor: "💁",
      description:
        "Now the customers walk in. HTML, CSS, and frameworks like React and Svelte are the face of your application — what the user actually sees and interacts with.",
  ...
  const layers = [
    {
      idx: 0,
      label: "CONCEPTS",
      href: "/topics/variables",
      subtitle: "Variables, functions, logic, OOP",
      sub: "The building itself",
      descBold: "Before anything else, the building must exist.",
      descRest:
        "Variables, functions, data structures, control flow, and object-oriented thinking are the concrete and steel of every application.",
      fill: "bg-slate-100/70",
      text: "text-slate-700",
      subColor: "text-slate-400",
    },
    {
      idx: 1,
      label: "SYNTAX & PRINCIPLES",
      href: "/syntax-styles/markup",
      subtitle: "Patterns, naming, DRY, KISS",
      sub: "The rules and policies",
      descBold: "Before the doors open, the bank sets its rules.",
      descRest:
        "Syntax styles, naming conventions, DRY, KISS, and planning are the policies that govern how every line of code behaves.",
      fill: "bg-amber-50/70",
      text: "text-amber-900",
    ...
    },
    {
      ...
      subtitle: "TypeScript, Python, Go, C#",
      sub: "The vault and ledger",
      descBold: "The vault is stocked and the ledger is open.",
      descRest:
        "Real languages — TypeScript, Python, Go, C# — plus version control, the command line, and local servers. This is where you store and manage real code.",
      fill: "bg-green-50/70",
      text: "text-green-800",
    ...
    },
    {
      idx: 3,
      label: "BUILD & SHIP",
      href: "/put-it-together/choose-your-stack",
      subtitle: "Stack, database, API, deploy",
      sub: "The employees and computers",
      descBold: "Now you hire the employees.",
      descRest:
        "Choose a stack, scaffold a project, connect a database, add auth, build an API, and deploy. These are the brains that make your application actually work.",
      fill: "bg-purple-50/70",
    ...
    },
    {
      ...
      href: "/user-interface/html-structure",
      subtitle: "HTML, CSS, React, Svelte",
      sub: "The customer experience",
      descBold: "Now the customers walk in.",
      descRest:
        "HTML, CSS, and frameworks like React and Svelte are the face of your application — what the user actually sees and interacts with.",
      fill: "bg-blue-50/70",
      text: "text-blue-800",
  ...
  let activeLayer = $derived(
    !completed ? layers.find((l) => l.idx === current) : null,
  );
  let pastLayers = $derived(
    layers.filter((l) => l.idx < current || (completed && l.idx === current)),
  );
</script>

<div
  style="width: 100vw; margin-left: calc(-50vw + 50%); margin-top: calc(-1.5rem - 1px);"
>
  <div>
    <!-- Video / illustration area -->
    <div
      class="relative w-screen min-h-[32rem] sm:min-h-[36rem] lg:min-h-[40rem] overflow-hidden"
    >
      <!-- Video/illustration backgrounds - all rendered, cross-fade via opacity -->
      {#each slides as s, idx}
        <div
          class="absolute inset-0 z-0 transition-opacity ease-in-out"
          style="opacity: {idx === current
            ? 1
            : 0}; transition-duration: {looping
            ? '1500ms'
            : fadeDuration + 'ms'};"
        >
          {#if s.image}
          ...
          {:else if s.id === "database"}
            <div
              class="flex w-full h-full items-center justify-center bg-slate-50 p-6"
            >
              <VaultAnimation />
            </div>
      ...
      {#each layers as layer (layer.idx)}
        <div
          class="absolute inset-0 z-20 font-mono transition-opacity duration-700 {layer.idx ===
          current
            ? 'opacity-100'
            : 'opacity-0 pointer-events-none'}"
        >
          <div
            class="w-full md:w-[22rem] h-full px-4 py-4 sm:px-6 sm:py-6 backdrop-blur-sm {layer.fill} flex flex-col gap-1"
          >
            <div class="hidden sm:flex items-center gap-2">
              ...
              <span class="text-sm font-bold leading-none {layer.text}"
                >{current + 1}</span
              >
              ...
              <button
                onclick={next}
                class="rounded-full p-1 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700 {pulsing
                  ? 'animate-pulse ring-2 ring-indigo-300'
                  : ''}"
                aria-label="Next"
              >
                <svg
            ...
            <span
              class="text-xl sm:text-xl font-bold text-black uppercase tracking-widest leading-none pt-4"
              style="font-family: 'Permanent Marker', sans-serif;"
              >Think Of Software Like A Bank</span
            >
            <span
              class="leading-none font-bold py-3 {layer.text}"
              ><a
                href={layer.href}
                class="text-2xl no-underline {layer.text} hover:underline"
                >{layer.label}</a
              ></span
            >
            <span
              class="text-xs sm:text-sm font-semibold leading-none {layer.text}"
              >({layer.sub})</span
            >
            <span
              class="text-xs sm:text-sm leading-relaxed font-bold {layer.text}"
              style="font-family: sans-serif;">{layer.descBold}</span
            >
            <span
              class="text-xs sm:text-sm leading-relaxed font-light {layer.text}"
              style="font-family: sans-serif;">{layer.descRest}</span
            >
            <span
              class="text-[10px] sm:text-xs font-light leading-tight {layer.text}"
              >({layer.subtitle.toUpperCase()})</span
            >
          </div>
        </div>
      {/each}
      ...
      <div
        class="absolute inset-0 z-20 font-mono transition-opacity duration-1000 {showSummary
          ? 'opacity-100'
          : 'opacity-0 pointer-events-none'}"
      >
        <div
        ...
        >
          ...
          <div class="absolute inset-0 pointer-events-none overflow-hidden">
            <div
              class="liquid-blob absolute rounded-full"
              style="width: 200px; height: 200px; top: -5%; left: -20%; background: radial-gradient(circle, rgba(99,102,241,0.2), transparent 70%);"
            ></div>
            <div
              class="liquid-blob absolute rounded-full"
              style="width: 250px; height: 250px; bottom: -10%; right: -30%; background: radial-gradient(circle, rgba(168,85,247,0.15), transparent 70%); animation-delay: -3s;"
            ></div>
            <div
              class="liquid-blob absolute rounded-full"
              style="width: 180px; height: 180px; top: 40%; left: 10%; background: radial-gradient(circle, rgba(34,197,94,0.12), transparent 70%); animation-delay: -6s;"
            ></div>
          </div>

          <div class="relative flex items-center justify-between">
            <p
              class="text-sm sm:text-base font-bold text-slate-700 font-mono tracking-wide"
            >
              The Full Stack
            </p>
            <button
          ...
          <!-- Stack diagram -->
          <div
            class="relative flex flex-col-reverse gap-0 rounded-xl overflow-hidden"
            style="background: rgba(255,255,255,0.2); border: 1px solid rgba(255,255,255,0.4);"
          >
            {#each layers as layer}
              <div
                class="w-full py-2 sm:py-2.5 px-3 sm:px-4 border-t first:border-t-0"
                style="border-color: rgba(255,255,255,0.3); background: linear-gradient(135deg, {layer.idx ===
                0
                  ? 'rgba(100,116,139,0.15), rgba(100,116,139,0.05)'
                  : layer.idx === 1
                    ? 'rgba(217,119,6,0.15), rgba(217,119,6,0.05)'
                    : layer.idx === 2
                      ? 'rgba(22,163,74,0.15), rgba(22,163,74,0.05)'
                      : layer.idx === 3
                        ? 'rgba(147,51,234,0.15), rgba(147,51,234,0.05)'
                        : 'rgba(59,130,246,0.15), rgba(59,130,246,0.05)'});"
              >
                <p class="text-xs sm:text-sm font-bold font-mono {layer.text}">
                  <a
                    href={layer.href}
                    class="no-underline {layer.text} hover:underline"
                    >{layer.label}</a
                  >
                </p>
                <p class="text-[9px] sm:text-[10px] font-light text-slate-500">
                  {layer.subtitle}
                </p>
              </div>
            {/each}
    ...
    <div
      class="flex sm:hidden items-center justify-center gap-2 bg-white/90 px-3 py-2 backdrop-blur-sm"
    >
      <button
        onclick={prev}
      ...
      <span class="text-sm font-bold leading-none {slide.textColor}"
        >{current + 1}</span
      >
      <span class="text-xs font-medium text-slate-400">of</span>
      ...
      <button
        onclick={next}
        class="rounded-full p-1.5 text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-700 {pulsing
          ? 'animate-pulse ring-2 ring-indigo-300'
          : ''}"
        aria-label="Next"
      >
