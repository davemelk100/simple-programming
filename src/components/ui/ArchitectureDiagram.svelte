  ...
  <div class="relative">
    ...
    <div class="mx-auto max-w-2xl overflow-hidden rounded-2xl">
      <div class="grid grid-cols-1 sm:grid-cols-2
        ...
        <!-- Left: text + band thumbnail -->
        <div class="flex flex-col justify-between p-5 sm:p-8">
          <div>
            <h3 class="flex items-center gap-2 text-xl sm:text-2xl {slide.textColor}" style="font-family: 'Permanent Marker';">
            ...
            <p class="mt-2 text-sm font-semibold italic text-slate-500">{slide.subtitle}</p>
          ...
          <div class="mt-4">
            {#key slide.id}
              {#if slide.id === 'infrastructure'}
                <img src="/lottie/infra-thumb.jpg" alt="Infrastructure" class="w-full h-auto rounded-lg object-cover shadow-sm border border-white/50"/>
              {:else if slide.id === 'config'}
                <img src="/lottie/config-thumb.avif" alt="Config" class="w-full h-auto rounded-lg object-cover shadow-sm border border-white/50"/>
              {:else if slide.id === 'database'}
                <img src="/lottie/database-thumb.jpg" alt="Database" class="w-full h-auto rounded-lg object-cover shadow-sm border border-white/50"/>
              {:else if slide.id === 'frontend'}
                <img src="/lottie/frontend-thumb.avif" alt="Frontend" class="w-full h-auto rounded-lg object-cover shadow-sm border border-white/50"/>
              {/if}
            {/key}
          </div>
        </div>
        ...
        <div class="flex items-stretch p-0">
          {#key slide.id}
            ...
            {:else}
              <LottiePlayer src={slide.lottie} class="w-full p-6" />
            {/if}
          {/key}
        </div>
      </div>
